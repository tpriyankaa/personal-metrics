...
function PageBanner() {
  return (
    ...
    <img src={PAGE_BANNER_IMG} alt="" className="w-full h-full object-cover" /> 
      <div className="absolute inset-0" style={{ background: "linear-gradient(180deg, rgba(5,5,5,0.15) 0%, rgba(5,5,5,0.35) 100%)" }} />
    </div>
  );
}
...
function CalendarView({ events, addEvent, deleteEvent }) {
  ...
  function submit(e) {
    e.preventDefault();
    if (!title.trim()) return;
    addEvent({ title: title.trim(), date: selected, time: time.trim() });
    setTitle("");
    setTime("");
    setShowForm(false);
  }

  const selectedEvents = eventsOn(selected).sort((a, b) => (a.time || "").localeCompare(b.time || ""));

  return (
    <div className="space-y-6">
      <SectionHeading title="Calendar" description="Organize upcoming deadlines, project milestones, and daily priorities in one place." />

      <div className="grid lg:grid-cols-[1fr_320px] gap-6">
        <div className="glass rounded-xl p-4">
          <div className="flex items-center justify-between mb-4">
            <button onClick={() => setCursor(new Date(year, month - 1, 1))} className="p-1.5 rounded-lg" style={{ background: "rgba(255,255,255,0.06)" }}>
              <ChevronLeft size={16} color={TEXT} />
            </button>
            <h3 className="font-display text-base font-semibold" style={{ color: TEXT }}>{monthLabel}</h3>
            <button onClick={() => setCursor(new Date(year, month + 1, 1))} className="p-1.5 rounded-lg" style={{ background: "rgba(255,255,255,0.06)" }}>
          ...
          <div className="grid grid-cols-7 gap-1 mb-1">
            {["S", "M", "T", "W", "T", "F", "S"].map((d, i) => (
              <div key={i} className="text-center font-mono text-[10px]" style={{ color: SUB }}>{d}</div>
            ))}
          </div>
            ...
            {cells.map((day, i) => {
              ...
              return (
                <button
                  ...
                  style={{
                    background: isSelected ? `linear-gradient(135deg, ${PURPLE_DEEP}, ${PURPLE})` : "transparent",
                    color: isSelected ? "#fff" : isToday ? PURPLE : TEXT,
                    border: isToday && !isSelected ? `1px solid ${PURPLE}` : "1px solid transparent",
                  }}
                >
                  {day}
                  {dayEvents.length > 0 && (
                    <span className="absolute bottom-1 w-1 h-1 rounded-full" style={{ background: isSelected ? "#fff" : PURPLE }} />
                  )}
                </button>
              );
            })}
        ...
        <div className="glass rounded-xl p-4">
          <div className="flex items-center justify-between mb-3">
            <h4 className="font-body text-sm font-medium" style={{ color: TEXT }}>{fmtDate(selected)}</h4>
            <button onClick={() => setShowForm((s) => !s)} className="p-1.5 rounded-lg" style={{ background: `linear-gradient(135deg, ${PURPLE_DEEP}, ${PURPLE})` }}>
          ...
          {showForm && (
            <form onSubmit={submit} className="space-y-2 mb-4">
              <input
                ...
                className="w-full px-3 py-2 rounded-lg text-sm outline-none font-body"
              ...
              <input
                ...
                style={{ background: "rgba(255,255,255,0.06)", border: `1px solid ${BORDER}`, color: TEXT }}
              />
              <button type="submit" className="w-full px-3 py-2 rounded-lg text-sm font-medium" style={{ background: `linear-gradient(135deg, ${PURPLE_DEEP}, ${PURPLE})`, color: "#fff" }}>
                Add event
              </button>
            </form>
          )}
          ...
          <div className="space-y-2">
            {selectedEvents.length === 0 && <EmptyNote text="No events on this day." />}
            {selectedEvents.map((e) => (
              <div key={e.id} className="flex items-center justify-between rounded-lg px-3 py-2" style={{ background: "rgba(255,255,255,0.04)" }}>
                <div>
                  <div className="text-sm" style={{ color: TEXT }}>{e.title}</div>
                  {e.time && <div className="font-mono text-[11px]" style={{ color: SUB }}>{e.time}</div>}
                </div>
                <button onClick={() => deleteEvent(e.id)} style={{ color: SUB }}>
                  <Trash2 size={14} />
                </button>
              </div>
            ))}
